...
def evaluate_model(model, batched_dataset, nb_class, args):
    ...
    if args['parallel']:
        print('Time : ' + str(datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + ' Process %d Reduce ' % hvd.rank(),
              flush=True)
        hvd.join()
...
def run_model(args):
    ...
    if debug:
        ...
        y = single_batch[args["label_type"] + "_labels_multi_hot"]
        y_ = teacher(x_all, training=False)
        # print(x_all)
        print('Teacher:')
        print(y)
        print(y_)
        y_ = student(x_all, training=False)
        # print(x_all)
        print('Student:')
        print(y)
        print(y_)
    ...
    def training_step(inputs, targets, first_batch):
        teacher_predictions = teacher(inputs, training=False)
        ...
        if args['parallel']:
            if first_batch:
                hvd.broadcast_variables(student.variables, root_rank=0)
                hvd.broadcast_variables(optimizer.variables(), root_rank=0)

        return loss_value, distillation_loss, y_pred
    ...
    student_checkpoint = tf.train.Checkpoint(model=student, optimizer=optimizer)
    # The main loop
    ...
    if args['mode'] == 'eval':
        ...
        checkpoint_test = tf.train.Checkpoint(model=student, optimizer=optimizer)
        checkpoint_test.restore(tf.train.latest_checkpoint(check_dir))
        test_eval = evaluate_model(student, test_batched_dataset, nb_class, args)
        (
            epoch_micro_precision,
            epoch_macro_precision,
            epoch_micro_recall,
    ...
    for epoch in range(args["nb_epoch"]):
        ...
        distill_epoch_loss_avg = tf.keras.metrics.Mean(dtype='float64')
        ...
        for i, single_batch in enumerate(batch_iterator):
            ...
            loss_value, distillation_loss, y_ = training_step(x_all, y, first_batch)
            # Track progress
            epoch_loss_avg.update_state(loss_value)  # Add current batch loss
            distill_epoch_loss_avg.update_state(distillation_loss)
            # Compare predicted label to actual label
            # y_ = model(x_all, training=False)
            # Update all custom metrics
            ...
            if i % 20 == 0 and args['worker_index'] == 0:
                ...
                print("Process {:01d}:  Epoch {:03d}: Iteration {:03d} Loss: {:.3f} Distillation Loss: {:.3f}".format(
                    args['worker_index'], epoch,
                    i, loss_value.numpy(), distillation_loss.numpy()))
        ...
        if epoch % 10 == 0 or epoch == args['nb_epoch'] - 1:
            ...
            print("Process {:01d}:  Epoch : {:03d}: Loss: {:.3f} Distillation Loss: {:.3f}".format(args['worker_index'],
                                                                                                   epoch,
                                                                                                   epoch_loss_avg.result(),
                                                                                                   distill_epoch_loss_avg.result()))
            # Evaluate model using the eval dataset
            evaluation = evaluate_model(student, val_batched_dataset, nb_class, args)
            ...
            if args['worker_index'] == 0 and f_score > bestfscore:
                ...
                print("Process {:01d}: New Best F-Score : {:.3f}\n Epoch : {:03d} Writing Checkpoint".format(
                    args['worker_index'], bestfscore, epoch))
                print("Process {:01d}:  Epoch : {:03d} Writing Checkpoint".format(args['worker_index'], epoch))
                student_checkpoint.save(student_checkpoint_dir)
                print(
            ...
            if args['worker_index'] == 0:
                ...
                print('Train took: {:0>2}hr. {:0>2}min. {:05.2f}sec.'.format(int(train_hours), int(train_minutes),
                                                                             train_seconds))
                print('Validation took: {:0>2}hr. {:0>2}min. {:05.2f}sec.'.format(int(eval_hours), int(eval_minutes),
                                                                                  eval_seconds))

            if epoch == args['nb_epoch'] - 1:
                checkpoint_test = tf.train.Checkpoint(model=student, optimizer=optimizer)
                checkpoint_test.restore(tf.train.latest_checkpoint(student_checkpoint_dir))
                test_eval = evaluate_model(student, test_batched_dataset, nb_class, args)
                (
                    epoch_micro_precision,
                    epoch_macro_precision,
